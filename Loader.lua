...
do
	local function install()
		...
		local minFarmValue = parseValue(Config.MinFarmValue) or 2000000
		...
		local function findLoot(free, origin)
			...
			local function consider(inst)
				...
				if focusLuck then
					...
					if farmValueFilter and value < minFarmValue then
						return
					end
					score = value
				end
		...
		MoneyBox:AddLabel("Min Value only farms crystals worth at least this. Example: 500k, 2m, 1.5b. Empty farms everything", true)

		local function setFarmMinValue(text)
			local parsed = parseValue(text)
			if not parsed then
				return
			end
			minFarmValue = math.max(parsed, 0)
			farmValueFilter = minFarmValue > 0
			Config.MinFarmValue = text
			loot = nil
		end

		MoneyBox:AddInput("FarmMinValue", {
			Text = "Min Value",
			Default = Config.MinFarmValue,
			Placeholder = "2m",
			Numeric = false,
			Finished = false,
